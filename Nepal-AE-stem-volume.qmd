...
Field measurements were split into XX group of data:

1.  Landscape indicators (see tab 01-info).
2.  Tree inventory of all trees in a 10 m radius of the selected tree for felling (see 02-tree-inventory).
3.  Sampled tree characteristics (see tab 03-tree)
4.  Stem diameter measurements (see tab 04-stem)
5.  Big branch diameter measurements (see tab 05-branch)
6.  Stem and big branch disc measurements (see tab 07-disc-stem)

For stem and big branch diameter measurements, diameter over and under bark were measured at 0.3, 0.8, 1.3, 1.8 and 2.3 m height then every meter (3.3, 4.3, etc.). When abnormal tree part were found (swollen, broken, etc.) additional diameter measurements were taken at the beginning middle and end of the abnormal sections.

A disc was cut at 0.8m, 1.8m, then 3.3 and every 2 meters. This was done regularly whether or not the disc portion was in or near an abnormal section.

![Stem measurements in the field](img/fieldwork-schema.png){.lightbox fig-alt="fieldwork-schema" fig-align="center"}

### Laboratory work

*Separating bark from wood, method for volume measurement, oven dry: specify oven temperature, and method to know when the samples are dry (weight stabilisation for ex., how many days).*
...
For wood logs in trees' stem and branches, from the trees' base to 10 cm, log dry weight were calculated based on log volume and disc wood density:

$$
W_{d, log_{i, j}} = V_{f, log_{i, j}} \times \rho_{b, j} 
$$
...
2.  Model development
    1.  For non-linear models with Maximum Likelihood approach, get starting parameter values. Starting values can come from the scientific literature or from log-transformed linear models for power models.
