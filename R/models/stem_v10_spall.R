...
##
tmp <- list()
tmp$missing <- tree |> filter(is.na(tree_stem_v10))

## + all trees ####
tree |>
  ggplot(aes(x = tree_dbh, y = tree_stem_v)) +
  geom_point() +
  geom_point(aes(y = tree_stem_v10), shape = 4, col = "darkred")

tree |>
  ggplot(aes(x = tree_d2h, y = tree_stem_v10)) +
  geom_point()
...
start <- coef(lm(log(V10) ~ log(D), data = dat_md))
start[1] <- exp(start[1])
# start <- c(0.001, 1)
...
spall_v10_d$md_info$AIC
spall_v10_d$graph
spall_v10_d$md_summary


## + V10 = a * D2H^b ####

start <- coef(lm(log(V10) ~ log(D2H), data = dat_md))
start[1] <- exp(start[1])
#start <- c(0.001, 1)
...
spall_v10_d2h$graph
spall_v10_d2h$md_summary

## + V10 = a * D2H^b, species group ####
...
start <- coef(lm(log(V10) ~ log(D/100) + log(H), data = dat_md))
start[1] <- exp(start[1])
# start <- c(0.001, 1)

md <- nlme(
  model = V10 ~ a * (D/100)^b * H^c, 
  data = dat_md,
  fixed = a + b + c ~ 1,
  groups = ~no_group,
  start = start, 
...
spall_v10_dxh$md_info$AIC
spall_v10_dxh$graph
spall_v10_dxh$md_summary


## + V10 = a * DBH^b * H^c, species ####
...
spall_v10_dxh_sp$data |>
  ...
  geom_line(data = spall_v10_d2h_sp$data, aes(y = pred), color = "darkgreen") +
  facet_wrap(~tree_species_code) +
  xlim(0, 40)
...
spall_v10 <- str_subset(ls(), pattern = "spall_v10_") %>%
  set_names() %>%                   
  map(~ get(.x)$md_info) %>%
  bind_rows(.id = "source") 
spall_v10

gg_spall_v10 <- str_subset(ls(), pattern = "spall_v10_") %>%
  set_names() %>%                   
  map(~ get(.x)$graph) %>%
